function subscribeNewsLetter(b, c, a) {
    ...
    if (enablePersonal == "true") {
        gender = $("[name=gender]:checked").val() == undefined ? 0 : $("[name=gender]:checked").val();
        postalCode = $("#txt-postal-code").val() == undefined ? "" : $("#txt-postal-code").val()
    }
    if (nonTrans) {
        var d = '{"email":"' + email + '","countryCode":"' + country + '","languageCode":"en","genderCode":' + gender + ',"zipCode":"' + postalCode + '"}';
        e = a
    } else {
        var d = '{"email":"' + email + '","firstName":"","lastName":"","gender":' + gender + ',"postalCode":"' + postalCode + '","kids":"' + kid + '","frequency":"' + frequency + '","birthYear":"","birthMonth":"","birthDay":"","country":"' + country + '"}'
    }
    $.ajax({
        type: "POST",
        url: e,
        ...
        success: function(i) {
            ...
            if (i) {
                var h = i.responseStatusCode;
                var j = i.responseAlreadyExists;
                var g = i.doubleOptInRequired;
                var f = i.responseInvalidCountry;
                if (h == "ok" && (f == false || nonTrans)) {
                    if (j == true) {
                        window.location = (thankYoupagePath + "?statusCode=successalreadyExist");
                        TealiumUtils.trackFunnelLocation("error_message", "newsletter_page")
                    } else {
                        if (g == true) {
                            window.location = (thankYoupagePath + "?statusCode=doubleOptInRequired");
                            ...
                            if (typeof(TealiumUtils) !== "undefined") {
                                TealiumUtils.newsletterSubscribed();
                    ...
                    if (nonTrans) {
                        window.location = (thankYoupagePath + "?statusCode=invalid")
                    } else {
...
function unsubscribeNewsLetter(b, a, d) {
    sessionStorage.setItem("newsletterSubmitted", "true");
    email = $("#txt-email").val();
    ...
    if (d != null && d != "") {
        var e = '{"email":"' + email + '","bpid":"' + d + '","frequency":"' + frequency + '"}'
    } else {
        var e = '{"email":"' + email + '","frequency":"' + frequency + '"}'
    }
    $.ajax({
        type: "POST",
        url: b,
        ...
        success: function(g) {
            sessionStorage.removeItem("newsletterSubmitted");
            if (g) {
                var f = g.responseStatusCode;
                if (f == "ok") {
                    window.location = (confirmUnsubscribePath + "?statusCode=success");
                    if (typeof(TealiumUtils) !== "undefined") {
                        TealiumUtils.newsletterUnsubscribed()
                    }
                } else {
                    if (f == "no-response") {
                        callUnsubscribeServlet(a, e, c, confirmUnsubscribePath)
                    } else {
                        if (c) {
                            window.location = (confirmUnsubscribePath + "?statusCode=error&email=true")
                        } else {
                            window.location = (confirmUnsubscribePath + "?statusCode=error")
                        }
                    }
                }
            }
...
function getThankYouPage(c) {
    var a = getPageFromPath(c);
    if (a == null || a == "") {
        a = window.location.pathname.replace("newsletter", "thank-you");
...
function subscribeNewsLetterWithPopup(b, c, a) {
    sessionStorage.setItem("newsletterSubmitted", "true");
    thankYoupagePath = getThankYouPage(c);
    enablePersonal = $("#enablePersonal").val();
    email = $("#txt-email").val();
    ...
    $.ajax({
        ...
        success: function(i) {
            ...
            if (i) {
                ...
                if (h == "ok" && f == false) {
                    if (j == true) {
                        thankYoupagePath += "?statusCode=successalreadyExist"
                    } else {
                        if (g == true) {
                            thankYoupagePath += "?statusCode=doubleOptInRequired"
                        } else {
                            thankYoupagePath += "?statusCode=success"
                        }
                    }
                } else {
                    if (nonTrans) {
                        thankYoupagePath += "?statusCode=invalid"
                    } else {
                        thankYoupagePath += "?statusCode=error"
                    }
                }
                if (!appeaser.Utils.isMobile() && $(".o-lightbox.is-open .lightbox-content").length > 0) {
                    $.ajax({
                        url: thankYoupagePath,
                        success: function(l) {
                            var k = $("<div/>")[0].innerHTML = l;
                            ...
                            if ($(".o-lightbox .lightbox-content").length > 1) {
                                $(".lightbox-content").first().html(l)
                            } else {
                                $(".lightbox-content").html(l)
                            }
                            if ((thankYoupagePath).includes("statusCode=success") && typeof(TealiumUtils) !== "undefined") {
                    ...
                    window.location = (thankYoupagePath)
                }
            }
        }
    })
...
function getLang() {
    var b = new RegExp(/[a-z]*_[A-Z]*/i),
        c = window.location.pathname.match(b),
        d = "en_eur",
        a;
    if (c && c.length) {
        a = c[0];
        if ((a.length > 4) && (a.length < 7) && (a.substr(2, 1) == "_")) {
            d = a
        }
    }
    return d
...
var readCookieValue = function(a) {
...
var getCountryCode = function() {
    var c = readCookieValue(COOKIE_COUNTRY_CODE),
        a = "";
    if (c) {
        var b = /(?:^[a-z]{2}_)?([A-Z]{2})$/.exec(c);
        if (b) {
            a = b[1]
        }
    }
...
function subscribeNewsLetterPopup(b, a) {
    sessionStorage.setItem("newsletterSubmitted", "true");
    enablePersonal = $("#enablePersonal").val();
    email = $("#txt-email").val();
    kid = $("#newsletter-i-am-kids").is(":checked");
    ...
    var d = b;
    if (typeof(TealiumUtils) !== "undefined") {
        TealiumUtils.newsletterPopupSubscribeStart();
        TealiumUtils.trackNewsletter()
    }
    if (!country) {
        country = ""
    ...
    if (nonTrans) {
        var c = '{"email":"' + email + '","countryCode":"' + country + '","languageCode":"en","genderCode":' + gender + ',"zipCode":"' + postalCode + '"}';
        d = a
    } else {
        var c = '{"email":"' + email + '","firstName":"","lastName":"","gender":' + gender + ',"postalCode":"' + postalCode + '","kids":"' + kid + '","frequency":"' + frequency + '","birthYear":"","birthMonth":"","birthDay":"","country":"' + country + '"}'
    }
    $.ajax({
        type: "POST",
        url: d,
        ...
        success: function(h) {
            ...
            if (h) {
                ...
                if (g == "ok" && e == false) {
                    if (j == true) {
                        var i = $(".newsletterEmailExistContent .m-error").html();
                        if (!$(".o-newsletter-popup .right-section .o-newsletter-padding .m-error.is-hidden").length > 0) {
                            $(".o-newsletter-popup .right-section .o-newsletter-padding .m-error").html(i)
                        } else {
                            $(".o-newsletter-popup .right-section .o-newsletter-padding .m-error").removeClass("is-hidden").html(i)
                        }
                    } else {
                        if (f == true) {
                            $(".is-newsletter-popup").addClass("newsletter-subscribe-popup");
                            var i = $(".newsletterOptInContent").html();
                            $(".o-newsletter-popup").html(i);
                            $(".newsletterPopUp-btn").click(function() {
                                $(".is-newsletter-popup .a-icon-close").trigger("click");
                ...
                $(".o-lightbox.newsletter-subscribe-popup").on("click", function(k) {
                    if ($(k.target).hasClass("newsletter-subscribe-popup")) {
                        $(".is-newsletter-popup").removeClass("newsletter-subscribe-popup")
                    }
                });
...
function arketNewsLetterPopup(b, a) {
    sessionStorage.setItem("newsletterSubmitted", "true");
    enablePersonal = $("#enablePersonal").val();
    ...
    $(".is-newsletter-ntn .a-icon-lg-close-circle").on("click", function() {
        $(".is-newsletter-ntn .newsletter-signup").show();
    ...
    $.ajax({
        ...
        success: function(k) {
            ...
            if (k) {
                ...
                if (l == "ok") {
                    if (q == true || g == true) {
                        var h = $(".o-newsletter-popup .email-value").find("#txt-email-id").data("alreadyexist-pattern-text");
                        ...
                        if (e == true) {
                            $(".o-newsletter-popup:not(.is-newsletter-ntn) .newsletter-signup").hide();
                            ...
                            if (nonTrans) {
                                $(".o-newsletter-popup.is-newsletter-ntn .newsletter-signup").hide();
                                if (!$(".subscription-confirmation-ntn").find(".a-image").length > 0) {
                                    $(".subscription-confirmation-ntn").append("<img class='a-image' src='/etc/designs/appeaser/p11/clientlibs/pattern-lib/images/newsletter-delivery-transparent.gif'><h2 class='a-heading-2'>" + f + "</h2><p class='a-paragraph'>" + o + "</p>")
                                }
                    ...
                    if (nonTrans) {
                        $(".o-newsletter-popup.is-newsletter-ntn .newsletter-signup").hide();
                        if (!$(".subscription-confirmation-ntn").find(".status-error").length > 0) {
                            $(".subscription-confirmation").append("<img class='a-image' src='/etc/designs/appeaser/p11/clientlibs/pattern-lib/images/newsletter-delivery-transparent.gif'><h2 class='a-heading-2 status-error'>" + n + "</h2><p class='a-paragraph'>" + p + "</p>")
                        }
                        $(".o-newsletter-popup.is-newsletter-ntn .subscription-confirmation").show()
                    } else {
                        $(".o-newsletter-popup:not(.is-newsletter-ntn) .newsletter-signup").hide();
...
function weekDayNewsLetterPopup(b, a) {
    var d = b;
    sessionStorage.setItem("newsletterSubmitted", "true");
    enablePersonal = $("#enablePersonal").val();
    ...
    $.ajax({
        ...
        success: function(j) {
            ...
            if (j) {
                ...
                if (i == "ok") {
                    if (k == true || f == true) {
                        var h = $(".o-newsletter-popup .no-description").find("#email-id-txt").data("alreadyexist-pattern-text");
...
function unsubscribeNewsLetterValidUser(c, g, d) {
    var e = getUrlParam("bpid", "");
    var b = getUrlParam("guid", "");
    confirmUnsubscribePath = getThankYouPage($("#unsubscribepage").val());
    var a = '{"guid":"' + b + '","bpid":"' + e + '","isNonTransactional":"' + d + '"}';
    if (b != "" & e != "") {
        var f = getGUIDValidation(g, h, a);
        
        function h(i) {
            if (i && d == "true") {
                window.location = (confirmUnsubscribePath + "?statusCode=successNT");
                if (typeof(TealiumUtils) !== "undefined") {
                    TealiumUtils.newsletterUnsubscribed()
                }
            } else {
                if (i) {
                    unsubscribeNewsLetter(c, g, e)
                } else {
                    window.location = (confirmUnsubscribePath + "?statusCode=error&email=true")
                }
            }
        ...
        if (e != "") {
            ...
            unsubscribeNewsLetter(c, g)
        }
    }
}

function getUrlParam(c, b) {
    var a = b;
...
function getGUIDValidation(c, b, a) {
    $.ajax({
        type: "POST",
        url: c,
        dataType: "json",
...
function callUnsubscribeServlet(c, d, a, b) {
    $.ajax({
        type: "POST",
        url: c,
        dataType: "json",
        ...
        success: function(f) {
            if (f) {
                var e = f.validEmail;
                if (e == "success" || e == "ok") {
                    window.location = (b + "?statusCode=success");
                    if (typeof(TealiumUtils) !== "undefined") {
                        TealiumUtils.newsletterUnsubscribed()
                    }
                } else {
                    if (a) {
                        window.location = (b + "?statusCode=error&email=true")
                    } else {
                        window.location = (b + "?statusCode=error")
                    }
                }
            }
        },
