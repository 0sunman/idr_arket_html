...
XRegExp = XRegExp || function (J) {
    ...
    var E, S, D, P = {natives: !1, extensibility: !1}, M = {
            exec: RegExp.prototype.exec,
            test: RegExp.prototype.test,
            match: String.prototype.match,
            replace: String.prototype.replace,
            split: String.prototype.split
        }, G = {}, L = {}, H = [], Q = "default", x = "class", q = {
            "default": /^(?:\\(?:0(?:[0-3][0-7]{0,2}|[4-7][0-7]?)?|[1-9]\d*|x[\dA-Fa-f]{2}|u[\dA-Fa-f]{4}|c[A-Za-z]|[\s\S])|\(\?[:=!]|[?*+]\?|{\d+(?:,\d*)?}\??)/,
            "class": /^(?:\\(?:[0-3][0-7]{0,2}|[4-7][0-7]?|x[\dA-Fa-f]{2}|u[\dA-Fa-f]{4}|c[A-Za-z]|[\s\S]))/
        }, T = /\$(?:{([\w$]+)}|(\d\d?|[\s\S]))/g, N = /([\s\S])(?=[\s\S]*\1)/g, W = /^(?:[?*+]|{\d+(?:,\d*)?})\??/,
        m = M.exec.call(/()??/, "")[1] === J, K = RegExp.prototype.sticky !== J, A = !1, B = "gim" + (K ? "y" : "");
    return E = function (d, o) {
        ...
        return function b(l, d) {
            for (var k = a[d].regex ? a[d] : {regex: a[d]}, h = [], g = function (e) {
        ...
        var g = /(\()(?!\?)|\\([1-9]\d*)|\\[\s\S]|\[(?:[^\\\]]|\\[\s\S])*]/g, b = 0, s, p, v = function (h, e, c) {
            var f = p[b - s];
            if (e) {
                if (++b, f) {
                    return "(?<" + f + ">"
    ...
    }, {scope: "all"}), D(/\[(\^?)]/, function (a) {
        return a[1] ? "[\\s\\S]" : "\\b\\B"
    }), D(/(?:\(\?#[^)]*\))+/, function (a) {
        return M.test.call(W, a.input.slice(a.index + a[0].length)) ? "" : "(?:)"
    }), D(/\\k<([\w$]+)>/, function (c) {
        var b = isNaN(c[1]) ? V(this.captureNames, c[1]) + 1 : +c[1], a = c.index + c[0].length;
...
p11App.shared.utils = p11App.shared.utils || new function () {
    ...
    l.ajaxInjectLightbox = function (n, m) {
        $.ajax({
            url: n, dataType: "html", success: function (o) {
    ...
    l.addToCartCall = function (m, r, t, o, n, s, p) {
        var q = {qty: r, productCodePost: t, product_osa_area: o, product_osa_type: n};
    ...
    l.addToCartCallData = function (p, m, q, o, s, n, t, r) {
        ...
        if (e) {
            e = false;
            $.ajax({
                url: m, type: "POST", data: p, success: function (v, x, w) {
                    t(v, q, u, o, x, w)
...
function ajaxQueryString(b, a) {
    $.ajax({
        ...
        data: JSON.stringify({productid: b, allCountries: a}),
        dataType: "json",
