...
l.ajaxInjectLightbox=function(n,m){$.ajax({url:n,dataType:"html",success:function(o){if(typeof m==="string"&&m.length>0){o=$(o).find(m)
...
$.ajax({url:o,success:function(t,v,u){if(typeof s==="function"){s()
}m.empty();
m.html(t);
...
}}function displaySiteBanner(){var b,e=null;
var g=new Date();
if(g){var c=g.getFullYear()+"/"+formatDate(parseInt(g.getMonth()+1))+"/"+formatDate(g.getDate())+" "+formatDate(g.getHours())+":"+formatDate(g.getMinutes())+":00";
var h=new Date(c);
var f=document.getElementById("sitebannersd");
var i=false;
if(f){b=new Date(document.getElementById("sitebannersd").value);
if(b&&b>h){i=true
}}var a=document.getElementById("sitebannered");
if(a){e=new Date(document.getElementById("sitebannered").value);
if(e&&e<=h){i=true
}}var d=document.getElementById("site-banner-id");
if(d){if(i){d.parentNode.removeChild(d)
}else{d.classList.remove("is-hidden")
...
}p11App.cart=new function(){var b=this;
...
b.loadMiniCart=function(){$.ajax({type:"GET",async:false,dataType:"html",url:window.p11App.cart.miniCartUrl()+"?x="+$.now(),success:function(d){$(".o-minicart").html(d)
},error:function(d,e,f){console.log("minicart error: "+e)
}})
};
b._showMiniCart=function(){$(".a-bag-icon").on("click",function(d){if($(".a-bag-icon").hasClass("is-active")){window.p11App.cart.loadMiniCart()
}})
};
b.startMiniCart=function(){if(Modernizr.mq(window.appeaser.Enums.MQ.IS_DESKTOP)){window.p11App.cart._showMiniCart();
window.p11App.cart.miniCartStarted=true
}};
b.miniCartStarted=false;
b.checkStartedMiniCart=function(){if(!window.p11App.cart.miniCartStarted){window.p11App.cart.startMiniCart()
}};
b.init=function(){$(window).bind("resize",function(){window.p11App.cart.checkStartedMiniCart()
});
$(document).ready(function(){window.p11App.cart.checkStartedMiniCart()
})
}
};
var mapStockStatus;
...
var productid;
var originalCountry="";
$(document).on("ready",function(){appeaser.subscribe(appeaser.Enums.listen.ON_LIGHTBOX_OPEN,function(){if($(".lightbox-content").find(".o-country").length){self.stockStatusAllCountries()
}})
});
function stockStatus(){if($("#size button.is-selected").length>0){productid=$("#size button.is-selected").data("code")
}else{productid=$(".uniqueSize").val()
}allCountries="";
if(mapStockStatus===undefined){ajaxQueryString(productid,allCountries)
}else{setDataStore(mapStockStatus)
}}function stockStatusAllCountries(){if($("#size button.is-selected").length>0){productid=$("#size button.is-selected").data("code")
}else{productid=$(".uniqueSize").val()
}allCountries="allCountries";
if(mapStockStatusAllCountry===undefined){ajaxQueryString(productid,allCountries)
}else{setDataStoreAllCountry(mapStockStatusAllCountry)
...
}function setDataStore(b){mapStockStatus=b;
b=[];
var a=$("#dataStoreCountry").find(".m-availability-store");
if(a&&a.length){a.remove()
}$.each(mapStockStatus,function(e){sizeSelected=productid.toString().substring(10,13);
if(sizeSelected==mapStockStatus[e].sizeCode){var c;
var d;
if(mapStockStatus[e]["traffLightInd"]=="G"){c="In stock";
d="in-stock"
}if(mapStockStatus[e]["traffLightInd"]=="A"){c="Only a few left";
d="few-left"
}if(mapStockStatus[e]["traffLightInd"]=="R"){c="Sold out";
d="sold-out"
...
d=[];
if(mapStockStatus.length){var b=mapStockStatus[0]["country"]
}var c="";
var a=$(".instore-label").find("[class^=m-availability-store]");
if(a&&a.length){a.remove()
}$.each(mapStockStatusAllCountry,function(g){sizeSelected=productid.toString().substring(10,13);
if(sizeSelected==mapStockStatusAllCountry[g].sizeCode){var e;
var f;
var h;
if(mapStockStatusAllCountry[g]["traffLightInd"]=="G"){e="In stock";
f="in-stock";
h="Available"
}if(mapStockStatusAllCountry[g]["traffLightInd"]=="A"){e="Only a few left";
f="few-left";
h="Low in stock"
}if(mapStockStatusAllCountry[g]["traffLightInd"]=="R"){e="Sold out";
f="sold-out";
h="Sold out"
}if(mapStockStatusAllCountry[g]["country"]!=c){c=mapStockStatusAllCountry[g]["country"];
if($(".o-country").length){$(".o-country").append('<div class="country-content" id="'+mapStockStatusAllCountry[g]["country"]+'"></div>')
}else{$(".lightbox-content").append('<div class="o-country"><div class="country-content" id="'+mapStockStatusAllCountry[g]["country"]+'"></div></div>')
...
$(window).load(function(){appeaser.subscribe(appeaser.Enums.trigger.SHOW_NOTIFICATION_AREA,function(c,b){if(b){if(c==="#notification-no-result"||(c==="#notification-undo"&&!b.undo)){var d=b.tags[0];
